...
pub const Token = struct {
    ...
    pub fn get_default() Token {
        return Token{ .type = .NOP, .start = 0, .end = 0, .line = "", .line_no = 0, .str = "" };
    }
};
...
fn get_test_fn(typ: TokenType) *const fn (u8) bool {
    return switch (typ) {
        ...
        .NAME => valid_name,
        .NUMBER => valid_number,
        .COMMENT, .NEWLINE => valid_new_line,
        .STRING => is_dQuote,
...
fn general_parse(self: *Self, typ: TokenType) !Token {
    ...
    const str = try self.consume_while(testFn);

    return Token{
        .type = typ,
        .start = start,
        .end = self.pos,
        .line = self.lines.items[self.line_no],
...
fn parse_tokens(self: *Self) !TokenArray {
    ...
    while (!self.eof()) {
        ...
        const token = try self.parse_token();
...
pub fn dump(tokens: []Token) !void {
    for (tokens) |token| {
        var str = token.str;
        if (eql(token.str, "\n")) str = "\\n";
        std.debug.print("{s:<10} str:   {s:<10} start: {d:<6} end: {d:<6} line_no: {d:<6} line: {s}\n", .{
            @tagName(token.type), str, token.start, token.end, token.line_no, token.line,
        });
    }
}
